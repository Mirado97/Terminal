"""
tui_real_mexc_futures.py — Arbitrage Terminal: Bybit Linear ↔ MEXC Futures.
Запуск: python3 tui_real_mexc_futures.py
"""
...
# MEXC Futures taker 6 bps × 2 + Bybit Perp taker 5.5 bps × 2 = ~23 bps round-trip
FEE_BPS = 23.0
...
def build_ui() -> Layout:
    ...
    layout["header"].update(Panel(
        Text.from_markup(
            ...
            f"MEXC[dim]perp[/]: {mexc_s} {_bal_str(_stats['mexc_usdt'])}{mx_r}  │  "
...
async def bot_main_real(symbols: list[str]) -> None:
    ...
    fee_table = FeeTable(overrides={
        (Exchange.MEXC,  MarketType.PERPETUAL): FeeSchedule(maker_bps=2.0, taker_bps=6.0),   # 0.06% taker
        (Exchange.BYBIT, MarketType.PERPETUAL): FeeSchedule(maker_bps=2.0, taker_bps=5.5),   # 0.055% taker
    })
    # MEXC Futures WS — реальный стрим, задержка ~10ms
        ...
        for buy_b, sell_b in [(updated, other), (other, updated)]:
            ...
            if pos is not None:
                ...
                if close_reason:
                    ...
                    bybit_fee = round(VIRTUAL_SIZE_USDT * 0.00055 * 2, 4)  # 0.055% × 2
                    mexc_fee  = round(VIRTUAL_SIZE_USDT * 0.0006  * 2, 4)  # 0.06% × 2
                ...
                # ── Вход ─────────────────────────────────────────────────
                # MEXC Futures поддерживает шорт — ограничение снято
            ...
            if by_key and by_sec:
                for acct in ("UNIFIED", "CONTRACT"):
                    try:
                        ...
                        pass
            # MEXC Futures баланс: contract.mexc.com/api/v1/private/account/assets
                ...
                try:
                    ...
                    for asset in data.get("data", []):
                        if asset.get("currency") == "USDT":
                            avail  = float(asset.get("availableBalance", 0) or 0)
                            frozen = float(asset.get("frozenBalance",    0) or 0)
                            val    = avail + frozen
...
async def fetch_symbols(n: int = 500, min_volume: float = 1_000) -> list[str]:
    """Топ N пар, доступных на Bybit Linear И MEXC Futures с объёмом > min_volume."""
    ...
    try:
        ...
        for t in mexc_data.get("data", []) if isinstance(mexc_data, dict) else []:
            raw = t.get("symbol", "")          # BTC_USDT
            sym = raw.replace("_", "")         # BTCUSDT
        ...
        print(f"  Загружено {len(symbols)} пар (Bybit Linear ∩ MEXC Futures >{min_volume:.0f} vol)")
        return symbols
...
async def main() -> None:
    ...
    symbols = await fetch_symbols(
        500,
        min_volume=getattr(_cfg, "MIN_MEXC_VOLUME_USDT", 1_000),
    )
